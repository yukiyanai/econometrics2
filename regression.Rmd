...
これらの理解に不安がある場合は、@asano-yanai2018 の第10--14章や[計量経済学の講義資料](http://yukiyanai.github.io/jp/classes/econometrics1/contents/) などを参照されたい。より厳密な説明は、計量経済学の教科書（たとえば、@ang2009、 @hansen2020、 @nishiyama2019、@sueishi2015 など）を参照。
...
`map()` や`sapply()` を使うほうが、`for` ループを使うより計算が早くなる。また、コードもシンプルに書けることが多い。
...
2つの変数 $Y$ と$D$ があり、この2変数の間に強い相関があるとする。このとき、$D$ が $Y$ の原因であるとは限らない。1つの可能性は、$Y$ が $D$ の原因であるというものである。因果の向きが逆の場合については、ここでは考えない（実際の研究では、これは重要な問題である）。
