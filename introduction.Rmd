...
.Rmd ファイルのサンプルを作ったので、必要なら以下のリンクからダウンロードして中身を確認されたい。ただし、ファイル名拡張子は .txt にしてある（.Rmd ファイルをダウンロードすると YAML ヘッダが消えてしまうため）ので、ダウンロードしたら `.txt` を `.Rmd` に変更して使うこと。

- macOS用：[jpntest_mac.txt](http://yukiyanai.github.io/jp/classes/econometrics2/docs/jpntest_mac.txt)
- Ubuntu用：[jpntest_ubuntu.txt](http://yukiyanai.github.io/jp/classes/econometrics2/docs/jpntest_ubuntu.txt)
- Windows用：[jpntest_win.txt](http://yukiyanai.github.io/jp/classes/econometrics2/docs/jpntest_win.txt)
- RStudio Cloud用：[jpntest_cloud.txt](http://yukiyanai.github.io/jp/classes/econometrics2/docs/jpntest_cloud.txt) （図の日本語は文字化けする）


RStudio Cloud を閉じる場合、ブラウザ（あるいはタブ）を閉じてもセッションが終了しないので、コンソールに`q("no")`と入力してRを終了しよう。
