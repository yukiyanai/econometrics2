...
（この資料を作成した時点での最新版は R 4.0.1 である。大学の情報演習室にインストールされているのが R 3.6.3 であり、今年度中は更新されないので、そちらに合わせる。2020年6月9日現在、自分のパソコンに新たにR
...
```{r, eval = FALSE}
ggsave(filename = "figs/japanese-test.pdf", plot = p1_jp,
       width = 4, height = 3)
```
...
```{r, eval = FALSE}
theme_set(theme_gray(base_size = 10, base_family = "IPAGothic"))
```
